...
help <- function(){
    cat("ChIP_makeMetaPeakPlot.R :
- Make an average profile plot where regions have been approximated to a certain size rather than centered on.\n")
    cat("Usage: \n")
    ...
    cat("--Type       : Tss, Tes, Peaks, metaPeak                                       [default = metaPeak]\n")
...
{
    oname = gsub(paste0('.', Type, ".*|", "_Tss.*|_Tes.*|_Peak.*|metaPeak.*"),".df",basename(foo[i]))
    oname <- gsub("-","_",oname)
    df <- get(load(foo[i]))
    assign(oname, df)
}
...
if( max(df[,grep("^x$",names(df),invert=TRUE)]) > 0.5 ){
    Ymax <- mround(max(df[,grep("^x$",names(df),invert=TRUE)]),0.5)
    Min <- 0
}else{
...
if(bins==0){
    ...
    Breaks <- c(0
               ,upStream/bins
               ,upStream/bins + approxLen/bins
               ,upStream/bins + approxLen/bins + upStream/bins)   
}
...
print({
    ...
    xlab("")+
    ylim(Min,Height)+
    ggtitle(paste(basename(outName), "\napproxLen", approxLen))+
